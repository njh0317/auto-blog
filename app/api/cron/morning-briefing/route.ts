...
[주제 다양성 규칙 - 매우 중요]
모닝 브리핑은 매일 비슷한 주제가 반복되지 않는 것이 중요합니다.
가능한 다양한 거시경제 이슈를 선택하세요.
...
중요 규칙:
- AI / 반도체 관련 이슈는 최대 1개까지만 허용
- 반드시 최소 1개 이상은 거시경제 이슈 포함 (예: 금리, 유가, 환율, 중국, 고용 등)
- 가능하면 기술주 외의 이슈를 우선적으로 고려하세요

[주제 선택 편향 방지]
최근 뉴스에서 AI / 반도체 관련 뉴스가 많이 보이더라도
항상 그 주제를 선택할 필요는 없습니다.

시장의 거시 변수(금리, 유가, 환율, 중국 경제 등)가
투자 판단에 더 중요한 경우 그 이슈를 우선적으로 선택하세요.

AI / 반도체 뉴스는 시장 전체 흐름에 영향을 주는 경우에만 선택하세요.
가능하면 기술주 외의 시장 이슈를 최소 1개 이상 포함하세요.

[작성 가이드]
1. Google 검색을 통해 최신 정보 확인
2. 오늘 가장 중요한 이슈 2~3개만 선정 (위 주제 다양성 규칙 준수)
3. 각 이슈에 대해:
   - 왜 중요한지
...
[제목 다양성 규칙 - 매우 중요]
같은 제목 패턴을 반복하지 마세요.
특히 아래 표현은 매일 사용하지 마세요:
- "출근길 5분"
- "꼭 봐야 할"
- "오늘 뭐가 중요할까"

가능한 다양한 제목 패턴을 사용하세요.
예시 패턴:
- "오늘 증시 핵심 이슈 정리"
- "오늘 시장에서 체크할 변수"
- "미국장 영향과 오늘 증시 포인트"
